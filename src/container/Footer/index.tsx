    ...
    <div className='app__footer app__flex'>
      <Stack direction={isMobile ? "column" : "row"} spacing={2} width={'100%'}>
        <div className='app__footer-desc'>
          ...
          <p>A passionate Frontend React Developer based in Lagos, Nigeria. <br/> Building the Frontend of Web and Mobile Applications, <br/> that leads to the success of the overall product.</p>
        </div>
          ...
          <ul>
            {
              footerItems?.map((item) => (
                <li>
                  <NextLink href={item.link}>{item.icon} &nbsp; {item.text}</NextLink>
                </li>
              ))
            }
          </ul>
